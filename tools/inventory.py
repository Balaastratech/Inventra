"""
Inventory Tools
Read-only tools for product and stock data.
"""
...
def calculate_stock_risk(
    ...
    - freshness check against stale_threshold_hours (defaults to this
      module's constant; callers with their own config -- e.g.
      submission.config.data_freshness_hours -- should pass it explicitly so
      there is one source of truth instead of two numbers that happen to
      agree today)
    """
    # Validate inputs
    ...
    # Zero observed demand has no finite days-of-cover meaning.  Do not invent
    # a velocity floor: that would manufacture a stockout date and can cause a
    # replenishment decision based on a fact we do not have.
    cover_days = available_units / daily_velocity if daily_velocity > 0 else (None if available_units > 0 else 0.0)
    ...
    return StockRisk(
        ...
        target_cover_days=target_cover_days,
        at_risk=(cover_days < target_cover_days if cover_days is not None else available_units <= 0) and not stale,
