"""
Vendor Tools
Read-only tools for vendor offers and performance metrics.
"""
...
def get_vendor_contact_email(vendor_id: str) -> Optional[str]:
    """Ops-maintained vendor contact address (Gap 8). Never populated from
    evidence/agent text -- this is the one read the vendor-send path (see
    submission/notifications/vendor_email.py) is allowed to use for the
    recipient address."""
    conn = _get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT contact_email FROM vendors WHERE vendor_id = ?", (vendor_id,))
    row = cursor.fetchone()
    conn.close()
    return row["contact_email"] if row and row["contact_email"] else None
    ...
    try:
        ...
        cursor.execute(
            ...
            SELECT vo.offer_id, vo.vendor_id, v.name, vo.unit_price,
                   vo.moq, vo.lead_time_days, vo.valid_until, v.active, vo.freight_flat, vo.freight_per_unit
        ...
        for row in all_rows:
            ...
            offers.append(VendorOffer(
                ...
                freight_flat=row["freight_flat"], freight_per_unit=row["freight_per_unit"],
            ))
...
def build_vendor_options(
    ...
    for offer in vendor_offers.offers:
        ...
        options.append(VendorOption(
            ...
            reliable=reliable,
            eligible=meets_deadline and reliable,
            freight_flat=offer.freight_flat,
            freight_per_unit=offer.freight_per_unit,
        ))
